...
readonly REQUIRED_BASE_COMMANDS="cat cut dirname grep locale mkdir sed tar tr which"
...
setup_directories() {
  ...
  readonly COLLECTOR_DIR="$SCRIPT_DIR/$collector_type"
  readonly OUTPUT_DIR="$COLLECTOR_DIR/output"
  readonly LOG_DIR="$COLLECTOR_DIR/log"
  readonly SQL_DIR="$COLLECTOR_DIR/sql"
  readonly TMP_DIR="$COLLECTOR_DIR/tmp"
  ...
  # Export for use in other scripts
  export SCRIPT_DIR COLLECTOR_DIR OUTPUT_DIR LOG_DIR SQL_DIR TMP_DIR LIB_DIR
}
...
check_requirements() {
  local required_commands="$REQUIRED_BASE_COMMANDS $*"
...
# Function to validate script is being run from correct location
validate_script_location() {
  local script_path="$1"
  local collector_type="$2"

  if [[ ! "$script_path" =~ /collector-5/${collector_type}/[^/]+$ ]]; then
    log_error "This script must be run from the collector-5/$collector_type directory"
    return 1
  fi
}
...
parse_args() {
  local -n args=$1  # Reference to associative array
  shift
